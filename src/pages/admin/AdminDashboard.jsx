...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: 'Closed', value: data.stats.closed, color: '#dc2626' },
  ];

  const chartData = [
    { name: 'Pending', value: data.stats.pending, color: STATUS_STYLES.Pending.background },
    { name: 'In Progress', value: data.stats.inProgress, color: STATUS_STYLES['In Progress'].background },
    { name: 'Resolved', value: data.stats.resolved, color: STATUS_STYLES.Resolved.background },
    { name: 'Closed', value: data.stats.closed, color: STATUS_STYLES.Closed.background },
  ];

  return (
    <AdminLayout>
      <div style={{ fontFamily: "'Segoe UI', sans-serif", color: '#1f2937' }}>
        <h1 style={{ color: '#1b3a57', margin: 0, fontSize: '1.6rem', fontWeight: 800 }}>Admin Dashboard</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.45fr) minmax(300px, 0.55fr)', gap: '18px' }}>
              ...
              <tbody>
                {data.recentComplaints.map((c) => (
                  <tr key={c.COMPLAINT_ID} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '10px 12px', ...getDepartmentTextStyle(c.DEPARTMENT) }}>{c.DEPARTMENT}</td>
                    <td style={{ padding: '10px 12px' }}>
                      <span style={{ ...(STATUS_STYLES[c.STATUS] || { background: '#64748b', color: 'white' }), display: 'inline-block', minWidth: '86px', textAlign: 'center', borderRadius: '999px', padding: '4px 10px', fontSize: '0.78rem', fontWeight: 700 }}>
                        {c.STATUS}
                      </span>
                    </td>
          ...
          {/* <div style={{ background: 'white', borderRadius: '14px', padding: '20px', boxShadow: '0 2px 10px rgba(0,0,0,0.07)' }}>
            <h2 style={{ marginTop: 0, color: '#1b3a57' }}>Status Breakdown</h2>
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={chartData} dataKey="value" nameKey="name" cx="50%" cy="46%" innerRadius={52} outerRadius={88} paddingAngle={3}>
                  {chartData.map((entry) => (
                    <Cell key={entry.name} 
                    fill={entry.color} />
                  ))}
...
>
  <h2 style={{ marginTop: 0, color: '#1b3a57' }}>
    Status Breakdown
  </h2>
