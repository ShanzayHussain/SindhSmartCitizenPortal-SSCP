...
function getMimeType(fileName = '') {
  ...
  const types = { pdf: 'application/pdf', png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', gif: 'image/gif', webp: 'image/webp', txt: 'text/plain' };
...
export default function AdminComplaints() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/admin/complaints/${selected.complaint_id}`)
      .then(async (res) => {
        ...
        if (!res.ok) throw new Error(detail?.message || 'Could not load complaint detail.');
        return detail;
      })
      .then((detail) => {
        setSelected(detail);
  ...
  const deptOfficers = useMemo(
    () => officers.filter((o) => Number(o.dept_id) === Number(selected?.dept_id)),
  ...
  async function saveUpdate() {
    ...
    try {
      ...
      const updated = { ...selected, ...(data?.complaint || {}), admin_remarks: form.admin_remarks };
      setSelected(updated);
      setForm({
      ...
      setComplaints((cur) => cur.map((c) => (c.complaint_id === updated.complaint_id ? { ...c, ...updated } : c)));
  ...
  async function openDocument(docId) {
    ...
    try {
      ...
      const blob     = base64ToBlob(data.file_data, getMimeType(fileName));
      const url      = URL.createObjectURL(blob);
      const opened   = window.open(url, '_blank', 'noopener,noreferrer');
      if (!opened) { const a = document.createElement('a'); a.href = url; a.download = fileName; a.click(); }
  ...
  return (
    <AdminLayout>
      <div style={{ fontFamily: "'Segoe UI', sans-serif", color: '#1f2937' }}>
        ...
        {error   && <p style={{ color: '#b91c1c', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '10px', padding: '10px 12px', marginBottom: '12px' }}>{error}</p>}
        {message && <p style={{ color: '#14863e', background: '#ecfdf5', border: '1px solid #bbf7d0', borderRadius: '10px', padding: '10px 12px', marginBottom: '12px' }}>{message}</p>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(460px, 560px)', gap: '16px', alignItems: 'start' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.86rem' }}>
                <thead>
                  <tr style={{ background: '#f8fafc' }}>
                    {['ID', 'Title', 'Citizen', 'Dept', 'Status'].map((h) => (
                      <th key={h} style={{ textAlign: 'left', padding: '10px 8px', color: '#334155', borderBottom: '1px solid #e2e8f0' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {complaints.length === 0 ? (
                    ...
                    const isSelected = selected?.complaint_id === c.complaint_id;
                    return (
                      <tr
                        key={c.complaint_id}
                      ...
                      >
                        <td style={{ padding: '10px 8px', borderBottom: '1px solid #f1f5f9', fontWeight: 800, color: '#1b3a57' }}>CMP-{String(c.complaint_id).padStart(5, '0')}</td>
                        <td style={{ padding: '10px 8px', borderBottom: '1px solid #f1f5f9' }}>{c.title}</td>
                        <td style={{ padding: '10px 8px', borderBottom: '1px solid #f1f5f9' }}>{c.citizen_name}</td>
                        <td style={{ padding: '10px 8px', borderBottom: '1px solid #f1f5f9', ...getDepartmentTextStyle(c.department) }}>{c.department}</td>
                        <td style={{ padding: '10px 8px', borderBottom: '1px solid #f1f5f9' }}><StatusBadge status={c.status} /></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <section style={{ background: 'white', borderRadius: '14px', padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.07)', border: '1px solid #e5e7eb', maxHeight: 'calc(100vh - 190px)', overflowY: 'auto' }}>
            ...
            {!selected ? (
              <p style={{ color: '#64748b' }}>Select a complaint from the list.</p>
            ) : detailLoading ? (
              <p style={{ color: '#64748b' }}>Loading complaint detail...</p>
            ) : (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '14px' }}>
                  <div>
                    <p style={{ margin: '0 0 4px', color: '#64748b', fontSize: '0.78rem', fontWeight: 800 }}>COMPLAINT #{String(selected.complaint_id).padStart(5, '0')}</p>
                ...
                <section style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '14px' }}>
                  <h4 style={{ margin: '0 0 10px', color: '#1b3a57' }}>Complaint Info</h4>
                  ...
                  <p style={{ margin: '12px 0 0', whiteSpace: 'pre-wrap', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px' }}>
                ...
                <section style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '14px' }}>
                  ...
                  <p style={{ margin: '10px 0 0' }}><strong>Address:</strong> {selected.citizen_address || '-'}</p>
                </section>

                <section style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '14px' }}>
                  <h4 style={{ margin: '0 0 10px', color: '#1b3a57' }}>Attachments</h4>
                  {(selected.documents || []).length === 0 ? (
                    <p style={{ color: '#64748b', margin: 0 }}>No documents attached.</p>
                  ) : (selected.documents || []).map((doc) => (
                    <button key={doc.doc_id} onClick={() => openDocument(doc.doc_id)}
                      style={{ display: 'block', border: '1px solid #bfdbfe', background: '#eff6ff', color: '#1d4ed8', padding: '9px 11px', borderRadius: '8px', cursor: 'pointer', marginBottom: '8px', width: '100%', textAlign: 'left', fontWeight: 700 }}>
                      📎 {doc.file_name || `Document ${doc.doc_id}`}
                    </button>
                  ))}
                </section>
                ...
                <section>
                  ...
                  <button disabled={saving} onClick={saveUpdate}
                    style={{ marginTop: '12px', background: saving ? '#64748b' : '#1b3a57', color: 'white', border: 'none', padding: '10px 14px', borderRadius: '8px', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: 800 }}>
