...
function getDepartmentTextStyle(department = '') {
  ...
  if (normalized.includes('ke')) return { color: '#f97316', fontWeight: 700 };
  if (normalized.includes('kelectric')) return { color: '#f97316', fontWeight: 700 };
  ...
  return (
    <AdminLayout>
      <div style={{ fontFamily: "'Segoe UI', sans-serif", color: '#1f2937' }}>
        ...
        {error ? <p style={{ color: '#b91c1c', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '10px', padding: '10px 12px', marginTop: 0, marginBottom: '12px' }}>{error}</p> : null}
        {message ? <p style={{ color: '#14863e', background: '#ecfdf5', border: '1px solid #bbf7d0', borderRadius: '10px', padding: '10px 12px', marginTop: 0, marginBottom: '12px' }}>{message}</p> : null}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(460px, 560px)', gap: '16px', alignItems: 'start' }}>
          ...
          <section style={{ background: 'white', borderRadius: '14px', padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.07)', border: '1px solid #e5e7eb', maxHeight: 'calc(100vh - 190px)', overflowY: 'auto' }}>
            ...
            {!selected ? (
              <p style={{ color: '#64748b' }}>Select a complaint.</p>
            ) : detailLoading ? (
